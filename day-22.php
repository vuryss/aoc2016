...
$input    = '/dev/grid/node-x0-y0     91T   71T    20T   78%
...
foreach ($input as $line) {
    preg_match('/node-x(\d+)-y(\d+).*?(\d+)T.*?(\d+)T.*?(\d+)T.*?(\d+)\%/', $line, $matches);
    $grid[$matches[2]][$matches[1]] = [
        'used'  => (int) $matches[4],
    ];
    $flat[] = [(int) $matches[4], (int) $matches[5]];

    if ($matches[6] == 0) {
        $emptyNodeSize = (int) $matches[5];
        $emptyCoords = [$matches[1], $matches[2]];
    }
}
...
for ($i = 0; $i < $nodes; $i++) {
    ...
    for ($j = 0; $j < $nodes; $j++) {
        if ($i != $j && $flat[$i][0] <= $flat[$j][1]) {
            $pairs++;
        }
    }
}

echo 'Pairs: ' . $pairs . PHP_EOL;

$moves = 0;

foreach ($grid as $y => $yColumn) {
    foreach ($yColumn as $x => $data) {
        if ($x == 0 && $y == 0) {
            $grid[$y][$x]['type'] = '+';
            echo str_pad($grid[$y][$x]['type'], 2, ' ');
            continue;
        }

        if ($x == count($yColumn) - 1 && $y == 0) {
            $grid[$y][$x]['type'] = '*';
            echo str_pad($grid[$y][$x]['type'], 2, ' ');
            continue;
        }

        if ($data['used'] > $emptyNodeSize) {
            $grid[$y][$x]['type'] = '#';
            echo str_pad($grid[$y][$x]['type'], 2, ' ');
            continue;
        }

        if ($x == $emptyCoords[0] && $y == $emptyCoords[1]) {
            $grid[$y][$x]['type'] = '_';
            echo str_pad($grid[$y][$x]['type'], 2, ' ');
            continue;
        }
...
// Part 2 - visual solving by viewing the grid
// 3 + 16 + 22 + 29 + (32 * 5)
// 19 + 22 + 29 + 160 = 230